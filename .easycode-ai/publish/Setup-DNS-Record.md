...
|---|---|---|---|---|
| A | @ | 216.150.1.1 | Auto/300s | Required: Make velocenow.com direct to the site |
| CNAME | www | bc200e827c59ccdc.vercel-dns-016.com. | Auto/300s | Optional: Make www.velocenow.com also accessible |
...
3. Add records one by one according to the table:
   ...
   - Host Record(Name):
     - Top-level Domain(such as velocenow.com) use "@"(some suppliers leave empty to represent @).
     - Subdomain(such as velocenow.com) use prefix "@".
...
- **Subdomain Access**: If the subdomain (such as blog.velocenow.com) is accessed, usually only the CNAME record in the table needs to be added.
